# <문제>
"""
최대 힙을 이용하여 다음과 같은 연산을 수행하라.
1) 배열에 자연수 x 추가.
2) 배열에서 가장 큰 값을 출력, 그 값을 배열에서 출력.
"""
...
"""
1) x가 자연수: 배열에 X 추가.
2) x가 0: 배열에서 가장 큰 값 출력 및 배열에서 제거.
...
class BinaryMaxHeap:
    ...
    def _percolate_down(self, cr) -> None:
        bg, le, ri = cr, cr*2, cr*2+1

        if le <= len(self) and self.items[le] > self.items[bg]:
            bg = le

        if ri <= len(self) and self.items[ri] > self.items[bg]:
            bg = ri

        if bg != cr:
            self.items[cr], self.items[bg] = self.items[bg], self.items[cr]
            self._percolate_down(bg)
